...
def compare_models(file_name: str, model_1: str, model_2: str, person_name: str, plot_name: str) -> None:
    """
    Compare two models from SentenceTransformer.
    """
    print(f"Comparing Models: Model 1: {model_1} and Model 2: {model_2}")
    ...
    for i in range(len(tm1)):
        plt.annotate(tm1[i][0][:6], (x[i], cosine_diff[i]), fontsize="6")
    ...
    for i in range(len(tm1)):
        plt.annotate(tm1[i][0][:6], (x[i], rank_diff[i]), fontsize="6")
        

    plt.suptitle(
        f"Difference in cosine similarity and ranking of everyone vs {person_name} for {model_1} and {model_2}", size=8
    )
    # plt.figure(figsize=(10,6))
    plt.savefig(os.path.join(C.BASE_RESULTS_PATH , plot_name))
